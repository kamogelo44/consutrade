...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    $action = $_GET['action'];
    updateProductStatus($conn, $product_id, $seller_id, $action);
...
$products = getSellerProducts($conn, $seller_id, $status, $search);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .filter-btn { padding: 8px 16px; border-radius: var(--radius-md); text-decoration: none; color: var(--gray-dark); background: var(--white); border: 1px solid var(--border-light); transition: all var(--transition-fast); }
        ...
        .btn-group a, .btn-group button { flex: 1; text-align: center; padding: 6px; border-radius: var(--radius-md); text-decoration: none; font-size: var(--font-sm); cursor: pointer; transition: all var(--transition-fast); border: none; }
        ...
        @media (max-width: 1024px) {
            .products-grid { grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: var(--spacing-md); }
        }
    </style>
</head>
<body>
...
<main class="seller-main-content">
    <div class="dashboard-content">
        <h1 style="margin-bottom: var(--spacing-lg); font-size: var(--font-2xl); font-weight: var(--font-bold)">My Products</h1>

        <!-- Action Bar -->
        <div style="display: flex; justify-content: space-between; margin-bottom: var(--spacing-lg); flex-wrap: wrap; gap: var(--spacing-md)">
            ...
            <form method="GET" style="display: flex; gap: var(--spacing-sm)">
                ...
                <input type="text" name="search" placeholder="Search products..." value="<?php echo htmlspecialchars($search); ?>" style="padding: 8px 12px; border-radius: var(--radius-md); border: 1px solid var(--border-light); width: 200px;">
                <button type="submit" style="padding: 8px 16px; background: var(--primary-color); color: white; border: none; border-radius: var(--radius-md); cursor: pointer;">Go</button>
                <?php if (!empty($search)): ?>
                    <a href="?status=<?php echo $status; ?>" style="padding: 8px 16px; background: var(--gray-bg-light); border-radius: var(--radius-md); text-decoration: none; color: var(--gray-dark);">Clear</a>
                <?php endif; ?>
            </form>
        </div>
        ...
        <div class="products-grid">
            <?php if (empty($products)): ?>
                <div style="text-align: center; padding: 60px; background: var(--white); border-radius: var(--radius-lg); border: 1px solid var(--border-light);">
            ...
            <?php else: ?>
                <?php foreach ($products as $product): ?>
                    <div class="product-card">
                        <img src="<?php echo getProductImageUrl($product['display_image'] ?? $product['image']); ?>" class="product-image" onerror="this.src='<?php echo $baseUrl; ?>images/default-product.png'">
