...
<head>
    ...
    <style>
        ...
        /* Stats Card */
        .profile-stats-card, .profile-edit-card {
            background: var(--white);
            border-radius: var(--radius-lg);
            border: 1px solid var(--border-light);
            padding: var(--spacing-xl);
        }
        .profile-stats-card h3, .profile-edit-card h3 {
        ...
        @media (max-width: 480px) {
            .profile-header-card, .profile-stats-card, .profile-edit-card, .quick-actions-section {
                padding: var(--spacing-md);
            }
            .profile-header-info h2 {
                font-size: var(--font-xl);
    ...
    $('#profileImageUpload').on('change', function() {
        ...
        if (file) {
            ...
            $.ajax({
                ...
                error: function() { showMessage('Error uploading image. Please try again.', true); }
            });
        }
    });
    ...
    $('#profileEditForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success) {
                    showMessage(data.message, false);
                    $('#profileHeaderName').text($('#fullName').val());
                } else {
                    showMessage(data.message, true);
                }
            },
            error: function() { showMessage('Error updating profile. Please try again.', true); }
        });
    });
